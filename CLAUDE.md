...
- Keyword generation: OpenAI chat completions on `OPENAI_MODEL` (default `gpt-5-mini`)
...
| File | What it does |
|---|---|
| `src/index.ts` | Everything: routes, data pull, scoring, HTML rendering, PDF, worker (~24,000 lines) |
| `src/lib/intel.ts` | Pure tested helpers extracted from index.ts: escapeHtml, money/date formatting, EDP parsing, consistency guard, aggregator filter, renewal radar, keyword matchers. Change against `src/lib/intel.test.ts` (`npm test`). |
...
`generateSearchKeywords()` calls OpenAI (`OPENAI_MODEL`) to produce accurate Contracts Finder search terms. Falls back to `buildKeywords()` only on failure. Never swap this order. The static `buildKeywords()` causes sector mismatches; LLM keywords are semantically accurate.
